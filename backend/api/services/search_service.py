...
"""
            ...
            def is_product_page(url: str) -> bool:
                ...
                if "jysk.co.uk" in url_lower:
                    ...
                    if len(path_parts) >= 4:  # More lenient - was 5
                        ...
                        if (
                            last_part
                            and len(last_part) > 10
                            and (
                                "-" in last_part
                                or any(char.isdigit() for char in last_part)
                            )
                        ):
                            return True
        ...
        if filtered_count > 0:
            print(
                f"🗑️  Filtered out {filtered_count} product(s) with £0 price (likely blog/non-product pages)"
            )
        ...
        type_matched_products = [
            p for p in valid_products if matches_product_type(p, user_pref.type)
        ]
        type_filtered = len(valid_products) - len(type_matched_products)

        if type_filtered > 0:
            print(
                f"🗑️  Filtered out {type_filtered} product(s) not matching type '{user_pref.type}':"
            )
        ...
        print("\n" + "=" * 80)
        print(
            f"✅ Successfully parsed {len(type_matched_products)} valid {user_pref.type} products (from {len(products)} total)"
        )
        print("=" * 80 + "\n")
